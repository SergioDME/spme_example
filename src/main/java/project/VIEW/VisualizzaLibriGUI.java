...
public class VisualizzaLibriGUI extends JFrame {
	...
	public VisualizzaLibriGUI(Controller c, List<RigaTabellaLibro> listaLibri) {
		setTitle("Library managment system");
		setIconImage(Toolkit.getDefaultToolkit().getImage(VisualizzaLibriGUI.class.getResource("/GUI/libri.png")));
		setResizable(false);

		controller = c;

		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 952, 465);
		...
		for(RigaTabellaLibro rigaTabella: listaLibri) {
			...
			vector.add(rigaTabella.getLibro().getTitolo());
			vector.add(rigaTabella.getLibro().getDoi());
			vector.add(rigaTabella.getLibro().getDescrizione());
			vector.add(rigaTabella.getLibro().getIsbn());
			if(rigaTabella.getLibro().getGenere().toString() == ("non_definito"))
				vector.add("non definito");
			else
				vector.add(rigaTabella.getLibro().getGenere().toString());
			vector.add(rigaTabella.getLibro().getNpagine());
			vector.add(rigaTabella.getLibro().getEditore());
			vector.add(rigaTabella.getLibro().getNumeroCitazioni());
			dati.add(vector);
		}
		...
		table.getColumnModel().getColumn(4).setPreferredWidth("titoloDelLibro".length()*10);
		table.getColumnModel().getColumn(5).setPreferredWidth("doiDelLibro".length()*15);
		table.getColumnModel().getColumn(6).setPreferredWidth("descrizione".length()*10);
		table.getColumnModel().getColumn(7).setPreferredWidth("1234567890123".length()*8);
		table.getColumnModel().getColumn(8).setPreferredWidth("RomanzoFormazione".length()*7);
		table.getColumnModel().getColumn(9).setPreferredWidth("Numero di pagine".length()*7);
		table.getColumnModel().getColumn(10).setPreferredWidth("EditoreDelLibro".length()*7);
		table.getColumnModel().getColumn(11).setPreferredWidth("citazioni ricevute".length()*7);
		table.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		table.setBounds(114, 11, 425, 347);

		JScrollPane scrollpane = new JScrollPane(table, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		scrollpane.setLocation(10, 5);
		scrollpane.setSize(916, 263);
		...
		esciButton.setBounds(806, 377, 120, 38);
		...
		btnVisualizza.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				for(RigaTabellaLibro rigaTabella: listaLibri) {
					if(rigaTabella.getLibro().getTitolo().equals(dettaglioField.getText())) {
						controller.openDettagliLibro(rigaTabella.getLibro());
						return;
					}
				}
		...
		btnNewButton.setBounds(463, 377, 89, 38);
		contentPane.add(btnNewButton);

		JLabel imageLabel = new JLabel("");
		imageLabel.setIcon(img);
		imageLabel.setBounds(0, 0, 936, 426);
